...
def fwd_selection(cluster, N):
	...
	#Note that N is the number of times the HSS is calculated using bootstrapping
        ...
        for rid in rids:
            ...
            elif cluster in ["0","1","2"]:
                df_scw = pd.concat([df_scw,remove_suspect_gusts(load_scws(rid)).query("cluster_new=="+str(cluster))],axis=0)
                df_null = pd.concat([df_null,load_nulls(rid).query("cluster_new=="+str(cluster))],axis=0)

	#Now undersample the Null dataframe, so there is an easier amount of data to handle.
	#There is still majority of nulls sampled, controlled by null_frac (> 1). 
	#For the original Brown and Dowdy SCW paper, null frac is something like 800, so we set null frac to 1,000 here
	#Note that this is only randomly sampled once, so results could be sensitive to this
	#If null_frac is None, then don't do any resampling
        if null_frac is None:
            df = pd.concat([df_null[list(preds)+["scw"]], df_scw[list(preds)+["scw"]]],axis=0).reset_index()
        else:
            df = pd.concat([df_null.sample(n=df_scw.shape[0] * null_frac, replace=True, random_state=seed)[list(preds)+["scw"]], df_scw[list(preds)+["scw"]]],axis=0).reset_index()
	...
	#Now save the output
        ...
        if pval_choice:
        	...
        	pval_str = "hss" 
        out_df.to_csv("/g/data/eg3/ab4502/ExtremeWind/skill_scores/logit_fwd_sel_cluster_"+str(cluster)+"_null_frac_"+str(null_frac)+"_seed_"+str(seed)+".csv")
